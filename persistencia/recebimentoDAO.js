...
export default class RecebimentoDAO {
    async gravar(recebimento, client) {
        if (recebimento instanceof Recebimento) {
            const sql = "INSERT INTO recebimentos(alu_nome, alu_rg, alu_observacoes, alu_dataNasc, alu_celular, alu_status, alu_motivoInativo) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING alu_codigo;";
            const parametros = [recebimento.nome, recebimento.rg, recebimento.observacoes, recebimento.dataNasc, recebimento.celular, recebimento.status, recebimento.motivoInativo];

            const retorno = await client.query(sql, parametros);
            recebimento.codigo = retorno.rows[0].alu_codigo;
        }
    }
